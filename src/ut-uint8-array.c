...
UtObject *ut_uint8_array_new_with_elements(size_t length, ...) {
  va_list ap;
  va_start(ap, length);
  UtObject *object = ut_uint8_array_new_with_va_elements(length, ap);
  va_end(ap);
  return object;
}

UtObject *ut_uint8_array_new_with_va_elements(size_t length, va_list ap) {
